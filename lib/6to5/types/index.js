...
var Set              = require("core-js/library").Set;
...
t.getIds = function (node, map, ignoreTypes, onlySet) {
  var search = [].concat(node);
  var ids    = object();

  if(onlySet && t.isIdentifier(node))
    search = [];
  ...
  while (search.length) {
    ...
    if(onlySet && t.isFor(id) && !t.isForStatement(id) && t.isVariableDeclaration(id.left)) {
      var addId = id.left.declarations[0].id;
      ids[addId.name] = addId;
    }

    if(onlySet && t.isVariableDeclarator(id) && !id.init) {
...
// can be called safely on any node, while getIds will return spurious ids on things that aren't declarations or assignment expressions
// NOTE: not recursive, you should call this on all nodes in a traversal
t.getIdsSafe = function (node, map, ignoreTypes, onlySet, parent) {
  if(onlySet && t.isFor(parent) && !t.isForStatement(parent) && t.isVariableDeclaration(node)) {
    return t.getIds(node, map, ignoreTypes, false);
  } else if(t.isDeclaration(node) || t.isAssignmentExpression(node)) {
    return t.getIds(node, map, ignoreTypes, onlySet);
  } else {
    return map ? {} : [];
  }
};
...
t.isReadIdentifier = function (node, parent) {
  return t.isReferencedIdentifier(node, parent) && !(t.isAssignmentExpression(parent) && parent.left === node && parent.operator === "=");
};

t.sideEffectFreeExpressions = new Set([
  "SequenceExpression",
...
/*
var sideEffectfulExpressions = new Set([
...
t.isMovementBarrier = function(node, parent) {
  if(t.isExpression(node) && !t.sideEffectFreeExpressions.has(node.type))
    return true;
  if(t.isStatement(node) && !t.isEmptyStatement(node) && !t.isExpressionStatement(node) && !t.isBlockStatement(node))
    return true;
  if(t.isVariableDeclarator(node) && (node.init || parent.kind !== "var"))
    return true;
  return false;
}

t.isExpressionResultUnused = function(node, parent, isParentSequenceExpressionResultUnused) {
  if(t.isExpressionStatement(parent))
    return true;
  if(t.isSequenceExpression(parent) && (parent.expressions[parent.expressions.length - 1] !== node || isParentSequenceExpressionResultUnused()))
    return true;
  if(t.isForStatement(parent) && parent.test !== node)
    return true;
  return false;
}
